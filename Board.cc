...
Board Board::next (const vector<Action>& asked, vector<Action>& done) const {
	...
	for (int turn = 0; turn < nb_players(); ++turn) {
		...
		switch (a) {
			...
			case Attacking:
			{
				Pos p = poquemons_[player].pos;
				if (b.exists_objective(p, d, poquemons_[player].scope) and not oneAttackRoundDone) {
	...
	for (int reps = 0; reps < 200 and not fnd; ++reps) {
		...
		for (int ii = -2; fnd and ii <= 2; ++ii)
			for (int jj = -2; fnd and jj <= 2; ++jj) {
				...
				fnd = (not pos_ok(p.i + ii, p.j + jj) ) or cell(p.i + ii, p.j + jj).id == -1;
				fnd = fnd and not is_ghost_wall(p, ghostWall_);
			}
		if (fnd) {
